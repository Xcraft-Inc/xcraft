...
const getRunnerConfig = function (runnerId, response, callback) {
  ...
  const config = require ('xcraft-core-etc') (null, response).load (
...
cmd.start = function (msg, response) {
  ...
  if (runners[runnerId]) {
    ...
    response.events.send (`ci.runners.start.${msg.id}.finished`);
...
cmd.stop = function (msg, response) {
  ...
  response.events.send (`ci.runners.stop.${msg.id}.finished`);
};
...
cmd.restart = function (msg, response) {
  ...
  response.events.send (`ci.runners.restart.${msg.id}.finished`);
};
...
cmd['create.prepare'] = function (msg, response) {
  ...
  response.events.send (`ci.runners.create.prepare.${msg.id}.finished`);
};
...
cmd['create.runner'] = function (msg, response) {
  ...
  ciClient.registerRunner (server.token, xcraft.platform, function (
    err,
    runner
  ) {
    if (err) {
      response.log.err ('Error during runner registration:', err);
      response.events.send (`ci.runners.create.runner.${msg.id}.finished`);
      response.events.send (`ci.runners.create.${msg.id}.finished`);
      return;
    }
...
cmd.delete = function (msg, response) {
  ...
  getRunnerConfig (runnerId, response, function (err, runnerConfig) {
    if (err) {
      response.log.err (err);
      response.events.send (`ci.runners.delete.${msg.id}.finished`);
      return;
    }
...
cmd.list = function (msg, response) {
  ...
  response.events.send (`ci.runners.list.${msg.id}.finished`);
};
