...
Tasks.prototype.make = function (callback) {
  ...
  this._response.command.send ('pacman.make', {
    packageArgs: [
      this._packages,
...
Tasks.prototype.isInstalled = function (callback) {
  ...
  self._response.events.subscribe ('pacman.status', function (msg) {
    self._response.events.unsubscribe ('pacman.status');
    isInstalled = msg.data.installed;
  });

  self._response.command.send ('pacman.status', {
...
Tasks.prototype.remove = function (callback, results) {
  ...
  this._response.command.send ('pacman.remove', {
...
Tasks.prototype.build = function (callback, results) {
  if (results.make !== this._response.events.status.succeeded || this._abort) {
    callback ();
    return;
  }

  this._response.command.send ('pacman.build', {
...
Tasks.prototype.disconnect = function (callback) {
  this._response.command.send ('disconnect', null, function () {
    callback ();
  });
};
