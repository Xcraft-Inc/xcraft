...
function Runner (runnerConfig, response) {
  if (!(this instanceof Runner)) {
    return new Runner (runnerConfig, response);
  }
...
Runner.prototype.start = function () {
  ...
  self.interval = setInterval (function () {
    ...
    if (!self.config.multi && self.builds.length) {
      return;
    }
...
Runner.prototype.getBuild = function () {
  ...
  this._client.getBuild (self.config, function (err, data) {
    if (err) {
      self.response.log.err ('error getting builds: %s', err);
      return;
    }
    if (!data) {
      return;
...
Runner.prototype.runBuild = function (data) {
  ...
  self.build = new Build (data, self.config, self.response);

  self.build.on ('done', function (err) {
    if (!err) {
      self.response.log.info ('build success [%s]', self.build.opts.id);
    } else {
      self.response.log.err ('build failed [%s]: %s', self.build.opts.id, err);
    }
