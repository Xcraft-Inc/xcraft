...
function Runner (runnerConfig) {
  ...
  this.log      = require ('xcraft-core-log') ('runner-' + this.config.id);
}
...
Runner.prototype.projectIsRunning = function (id) {
  ...
  return Object.keys (self.build).some (function (key) {
...
Runner.prototype.getBuild = function () {
  ...
  client.getBuild (self.config, function (err, data) {
    if (err) {
      self.log.err ('error getting builds', err);
      return;
    }
    if (!data) {
      return;
...
Runner.prototype.runBuild = function (data) {
  ...
  self.build.on ('done', function (success) {
    if (success) {
      self.log.info ('build success [%s]', self.build.opts.id);
    } else {
      self.log.err ('build failed [%s]', self.build.opts.id);
    }
