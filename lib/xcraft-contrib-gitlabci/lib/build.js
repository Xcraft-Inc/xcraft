  ...
  xcraftPortal.connect (null, function () {
    ...
    async.auto ({
      make: function (callback) {
        xcraftPortal.command.send ('pacman.make', args, function (err, msg) {
          callback (null, msg.data);
        });
      },

      isInstalled: ['make', function (callback) {
        var isInstalled = false;

        xcraftPortal.events.subscribe ('pacman.status', function (msg) {
          xcraftPortal.events.unsubscribe ('pacman.status');
          isInstalled = msg.data.installed;
        });

        xcraftPortal.command.send ('pacman.status', {
          packageRefs: mainPackage + '-src'
        }, function () {
          callback (null, isInstalled);
        });
      }],

      remove: ['isInstalled', function (callback, results) {
        if (!results.isInstalled) {
          callback ();
          return;
        }

        xcraftPortal.command.send ('pacman.remove', {
          packageRefs: mainPackage + '-src'
        }, function () {
          callback ();
        });
      ...
      build: ['remove', function (callback, results) {
        if (results.make !== xcraftPortal.events.status.succeeded) {
          callback ();
          return;
        }

        xcraftPortal.command.send ('pacman.build', {
          packageRefs: mainPackage
        }, function (err, msg) {
          callback (null, msg.data);
        });
      }],

      disconnect: ['build', function (callback) {
        xcraftPortal.command.send ('disconnect', null, function () {
          callback ();
        });
      }],

      stop: ['disconnect', function (callback) {
        xcraftPortal.stop (callback);
      }]
