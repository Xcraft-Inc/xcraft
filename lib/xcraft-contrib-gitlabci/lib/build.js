'use strict';

var moduleName = 'gitlabci/build';

var async        = require ('async');
var path         = require ('path');
var util         = require ('util');
var EventEmitter = require ('events').EventEmitter;

var client = require ('./client.js') ();
var Tasks  = require ('./build-tasks.js');
var xLog   = require ('xcraft-core-log') (moduleName);
var log    = require ('xcraft-core-utils').log;


function Build (opts, runnerConfig) {
  if (!(this instanceof Build)) {
    return new Build (opts, runnerConfig);
  }

  EventEmitter.call (this);
  this.config   = runnerConfig;
  this.buildDir = this.config.root;
  opts.commands = opts.commands || [];
  opts.timeout  = +(this.config.timeout || opts.timeout || 5000) * 1000;
  this.opts     = opts;
  this.output   = '';
  this.dirty    = false;
  this.retry    = 0;
  this.projectDir = path.join (this.buildDir, 'project-' + opts.projectId);
  this.state = 'waiting';
}

util.inherits (Build, EventEmitter);

Build.prototype.run = function () {
  var self = this;

  this.state = 'running';

  var cmds = this.opts.commands;
  var dir  = this.projectDir;

  self.runCommand (cmds, dir, function (err) {
    self.state = err ? 'failed' : 'success';
    self.update (function () {
      self.emit ('done', err);
    });
  });
};

Build.prototype.append = function (str) {
  this.output += str;
  this.dirty = true;
};

 /* Update the build on GitLab CI.
  *
  * If the status code is not right (like 200), the runner retries at least 10
  * times. It is aborted immediatly only when a low level error occures.
  */
Build.prototype._tryToUpdate = function (callback) {
  var self = this;

  self.update (function (err, updated) {
    if (!updated) {
      self.retry++;
    } else {
      self.retry = 0;
    }

    if (self.retry) {
      xLog.info ('retried to contact CI %d/10', self.retry);
    }

    if (err || self.retry === 10) {
      if (!err) {
        err = 'the runner has retried to contact CI ' + self.retry + ' times';
      }

      if (callback) {
        callback (err);
      }
      return;
    }

    self.dirty = !updated;
    if (callback) {
      callback ();
    }
  });
};

Build.prototype.runCommand = function (cmd, dir, callback) {
  var self = this;

  var packages    = cmd[0] || '';
  var mainPackage = packages.split (',')[0].replace (/:.*/, '');

  var logWidth = self.config.logWidth || 120;

  /* TODO: add timeout support. */

  var xBusClient = require ('xcraft-core-busclient');
  var xcraftPortal = new xBusClient.BusClient (self.config.xcraft);

  var tasks = new Tasks (xcraftPortal, mainPackage, packages, self.opts.sha);

  /* Look for an update every updateInterval [ms]. */
  var timer = setInterval (function () {
    if (!self.dirty || tasks.getAbort ()) {
      return;
    }

    self._tryToUpdate (function (err) {
      if (err) {
        tasks.setAbort (true, err);
      }
    });
  }, self.config.updateInterval || 3000);

  xcraftPortal.connect (null, function () {
    var prevPercent = -1;

    ['verb', 'info', 'warn', 'err'].forEach (function (level) {
      xcraftPortal.events.subscribe ('widget.text.' + level, function (msg) {
        if (/^gitlabci(|\/.*)$/.test (msg.data.mod)) {
          return;
        }

        var text = log.decorate (level, 'CI', msg.data.mod, msg.data.text, logWidth);
        self.append (text + '\n');
      });
    });

    xcraftPortal.events.subscribe ('widget.progress', function (msg) {
      var ratio   = msg.data.position / msg.data.length;
      var percent = parseInt (100 * ratio);
      if ((percent % 2) !== 0) {
        return;
      }

      if (percent === prevPercent) {
        return;
      }

      prevPercent = percent;

      var progress = util.format ('%s |%s> %d\%',
                                  msg.data.topic,
                                  new Array (percent / 2).join ('='),
                                  percent);
      var text = log.decorate ('info', 'CI', msg.data.mod, progress, logWidth);

      self.append (text + '\n');
    });

    xLog.info ('runner connected with ' + xcraftPortal.getOrcName ());

    async.auto ({
      make:        tasks.make.bind (tasks),
      isInstalled: ['make',        tasks.isInstalled.bind (tasks)],
      remove:      ['isInstalled', tasks.remove.bind (tasks)],
      build:       ['remove',      tasks.build.bind (tasks)],
      disconnect:  ['build',       tasks.disconnect.bind (tasks)],
      stop:        ['disconnect',  tasks.stop.bind (tasks)]
    }, function (err, results) {
      clearInterval (timer);

      if (results.make  !== xcraftPortal.events.status.succeeded ||
          results.build !== xcraftPortal.events.status.succeeded) {
        if (!err) {
          err = '';
        }

        err += util.format ('\nmake status: %d\nbuild status: %d',
                            results.make, results.build);
      }

      xcraftPortal = null;

      if (tasks.getAbort ()) {
        if (err) {
          err += '\n' + tasks.getAbortError ();
        } else {
          err = tasks.getAbortError ();
        }
        callback (err);
        return;
      }

      self._tryToUpdate (function (_err) {
        if (err && _err) {
          err += '\n' + _err;
        }
        callback (err || _err);
      });
    });
  });
};

Build.prototype.update = function (callback) {
  var id = this.opts.id;
  client.updateBuild (this.config, id, this.state, this.output, callback);
};

module.exports = Build;
