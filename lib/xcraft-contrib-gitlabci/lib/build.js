...
function Build(opts, runnerConfig, resp) {
  if (!(this instanceof Build)) {
    return new Build(opts, runnerConfig, resp);
  }
...
Build.prototype.runCommand = function (cmd, dir, callback) {
  ...
  xcraftPortal.connect(null, function () {
    ...
    self.resp.log.info('runner connected with ' + xcraftPortal.getOrcName());
    ...
    async.auto(
      ...
      function (err, results) {
        ...
        xcraftPortal.stop((err) => {
          if (
            results.make !== resp.events.status.succeeded ||
