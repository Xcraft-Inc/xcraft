...
cmd.build = function(cmd, resp) {
  ...
  resp.log.info('xdk goblin in ' + config.home);
  ...
  async.series(
    {
      clean: function(callback) {
        ...
        resp.log.info('xdk goblin in ' + gadgetPath);
        var gadgetModules = path.join('.', '/node_modules/');
        resp.log.info('lookin for ' + gadgetModules);
        if (fs.existsSync(gadgetModules)) {
          resp.log.info('xdk goblin cleaning ' + gadgetModules);
 ...
 * @param {Object} cmd - gadgetName
 */
cmd.craft = function(cmd, resp) {
  var gadgetName = cmd.data.gadgetName;
  var gadgetPath = path.join(config.home, gadgetName);
  var oldPath = process.cwd();

  resp.log.info('using ' + config.home);
  resp.log.info(gadgetName);
  ...
  async.series(
    {
      ...
      craft: function(callback) {
        ...
        process.chdir(gadgetPath);
        resp.log.info('TODO: craft code here' + gadgetPath);
        callback();
      },
    },
    function(err) {
      ...
      process.chdir(oldPath);
      busClient.events.send(`xdk.goblin.craft.${msg.id}.finished`);
    }
  );
};
...
cmd.digin = function() {
  ...
  busClient.events.send(`xdk.goblin.digin.${msg.id}.finished`);
};
...
cmd.run = function(msg, resp) {
  ...
  xProcess.spawn(xatomCmd, [goblinPath], {}, function(err) {
    ...
    busClient.events.send(`xdk.goblin.run.${msg.id}.finished`);
  });
};
 ...
 * (from /home/xdk).
 */
cmd.test = function(msg, resp) {
  var devGoblinPath = path.join(xConf.xcraftRoot, config.home, 'xcraft-goblin');

  resp.log.info('xdk test goblin is in ' + devGoblinPath);
  var xatomCmd = 'xcraft-atom' + xPlatform.getCmdExt();
  ...
  xProcess.spawn(xatomCmd, [devGoblinPath], {}, function(err) {
    ...
    busClient.events.send(`xdk.goblin.test.${msg.id}.finished`);
  });
};
...
cmd.install = function(msg) {
  ...
  var installGoblin = function(callback) {
    busClient.events.subscribe(`pacman.install.${msg.id}.finished`, function() {
      busClient.events.unsubscribe(`pacman.install.${msg.id}.finished`);
      callback();
    });
  ...
  async.eachSeries(
    ...
    function(pkg, callback) {
      busClient.events.subscribe(`pacman.make.${msg.id}.finished`, function() {
        busClient.events.unsubscribe(`pacman.make.${msg.id}.finished`);
        callback();
      });
    ...
    function() {
      installGoblin(function() {
        installGui(function() {
          busClient.events.send(`xdk.goblin.install.${msg.id}.finished`);
        });
      });
    }
  );
