...
cmd.build = function (cmd, response) {
  ...
  if (!fs.existsSync (gadgetPath)) {
    ...
    response.log.err ('Unable to build gadget ' + gadgetName + ' , not found in: ' + config.home);
    return;
  }
  ...
  const xProcess  = require ('xcraft-core-process') ({logger: 'xlog', response: response});
  ...
  async.series ({
    ...
    install: function (callback) {
      process.chdir (gadgetPath);
      xProcess.spawn (installCmd, ['install', '.', '--registry', registryArg], {}, callback);
    },
