...
`;
    ...
    yield xProcess.spawn(
      'gpg',
      ['--batch', '--gen-key', keyDetailsPath],
      {cwd: process.env.GNUPGHOME},
      next
    );
    /* Set trust to 5 for the key so we can encrypt without prompt. */
    yield xProcess.spawn(
      `echo "5\ny\n" | gpg --command-fd 0 --expert --edit-key ${distribution}@localhost trust;`,
      [],
    ...
    yield xProcess.spawn('gpgconf', ['--kill', 'gpg-agent'], {}, next);
  }
});
...
Goblin.registerQuest(goblinName, 'initialize', function* (
  ...
  const gpgKey = yield quest.me._generateKey({distribution});
...
Goblin.registerQuest(goblinName, 'publishPackage', function* (
  ...
  const files = xFs.lsfile(sourceDeb, new RegExp(`^${packageName}.*\\.deb$`));
