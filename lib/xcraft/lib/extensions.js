...
var packagesFilter = function (modules) {
  ...
  var notFound = [];
  modules.forEach (function (pkg, index) {
  ...
  /* Fuzzy finder */
  notFound.forEach (function (pkg) {
    packages.every (function (item) {
      if (new RegExp ('.*' + pkg.name + '.*').test (item)) {
        modules[pkg.index] = item;
        console.log ('[' + moduleName + '] Info: the provided package \'' +
                     pkg.name + '\' has been replaced by \'' + item + '\'');
        return false;
      }
