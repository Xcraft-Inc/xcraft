...
var unpmConfigFile = fs.readFileSync (path.resolve ('./etc/unpm/config.json'), 'utf8');
var unpmConf       = JSON.parse (unpmConfigFile);
...
var startUnpm = function () {
  ...
  if (!isRunning) {
    ...
    var unpmFork = spawn ('node', [path.resolve (__dirname, './unpmServer.js')], {
      detached: true,
...
cmd.publish = function (modules, callback) {
  ...
  async.eachLimit (packages, 4, function (packageToPublish, callback) {
    publish (packageToPublish, true, unpmConf.host.hostname, unpmConf.host.port, callback);
  },
  function () {
    stopUnpm ();
    callback ();
...
cmd.unpublish = function (modules, callback) {
  ...
  startUnpm ();
  var server = unpmConf.host.hostname;
  var port   = unpmConf.host.port;
...
cmd.cache = function (args, callback) {
  var unpmService = startUnpm ();

  cache (unpmConf.fallback, unpmConf.host.hostname, unpmConf.host.port, function () {
    stopUnpm ();
    callback ();
  });
};
...
cmd.install = function (modules, callback) {
  ...
  install (packages, true, unpmConf.host.hostname, unpmConf.host.port, function () {
    stopUnpm ();
    callback ();
  });
};
