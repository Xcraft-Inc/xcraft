...
var publish = function(packageToPublish, isDir, hostname, port, callback) {
  console.log(
    '[' + moduleName + '] Info: publishing ' + packageToPublish + ' in NPM'
  );
  ...
  try {
    ...
    var packagePath = isDir
      ? path.resolve('./lib/', packageToPublish)
      : packageToPublish;

    if (
      path.extname(packagePath) !== '.tgz' &&
      ...
      console.log(
        '[' +
          ...
          '] Info: ' +
          packagePath +
          ' is not a valid npm package, skipping'
      );
      callback();
      return;
    }
...
var resolveTarball = function(packageWithVersion, callback) {
  var args = ['view', packageWithVersion, 'dist', '--json'];
...
cmd.publish = function(modules, callback) {
  ...
  async.eachLimit(
    packages,
    4,
    function(packageToPublish, callback) {
      publish(
        packageToPublish,
        true,
        unpm.conf.hostname,
...
cmd.unpublish = function(modules, callback) {
  ...
  var runUnpublish = function(list, callback) {
    async.eachLimit(
      list,
      4,
      function(packageToUnPublish, callback) {
  ...
  async.eachLimit(
    packages,
    4,
    function(packageToView, callback) {
...
/**
 * Run startcraft via `npm install`.
 */
cmd.startcraft = function(callback) {
  install(null, false, null, null, callback);
};
...
cmd.verify = function(modules, callback) {
  ...
  packages.forEach(function(p) {
    var libFile = fs.readFileSync(
      path.resolve('./lib/', p, 'package.json'),
      'utf8'
    );
    var installedFile = fs.readFileSync(
      path.resolve('./node_modules/', p, 'package.json'),
      'utf8'
    );
    ...
    if (
      parseInt(libVersion[0]) > parseInt(installedVersion[0]) ||
...
cmd.ncu = function(modules, callback) {
  var ncu = require('npm-check-updates');

  console.log('[' + moduleName + '] Info: check modules updates');

  var packages = packagesFilter(modules);

  /* Check the main Xcraft package.json. */
  packages.push('xcraft/deps');
  ...
  async.eachSeries(
    packages,
    function(p, callback) {
      process.chdir(path.resolve(cwd, './lib', p));

      var def = path.resolve('package.json');
      var data = fs.readFileSync(def);
      var obj = JSON.parse(data);

      // ncuOpts.packageData  = data;
      ncuOpts.packageFile = 'package.json';
      ncuOpts.jsonUpgraded = true;
      ncuOpts.silent = true;
      ...
      ncu.run(ncuOpts).then(function(upgraded) {
        Object.keys(upgraded).forEach(function(mod) {
          console.log(
            '[' + moduleName + '] Info: - %s %s%s -> %s',
            mod,
            new Array(26 - mod.length).join(' '),
            (obj.dependencies ? obj.dependencies[mod] : '?') ||
              (obj.devDependencies ? obj.devDependencies[mod] : '?') ||
              (obj.peerDependencies ? obj.peerDependencies[mod] : '?'),
            upgraded[mod]
          );
        });
...
opt['ncu-ua'] = function(args, callback) {
  ncuOpts.upgradeAll = true;
  callback();
};
