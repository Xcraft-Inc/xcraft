...
cmd.bin = (bins, callback) => {
  ...
  try {
    bins.forEach ((bin) => {
      const location = which.sync (bin);
      try {
        fs.symlinkSync (location, path.join (binDir, bin), 'junction');
      } catch (ex) {
        if (ex.code !== 'EEXIST') {
          throw ex;
        }
...
cmd.ncu = function (modules, callback) {
  ...
  async.eachSeries (packages, function (p, callback) {
    ...
    console.log ('[' + moduleName + '] Info: %s ...', p === '..' ? 'Xcraft root' : p);
    ncu.run (ncuOpts).then (function (upgraded) {
      Object.keys (upgraded).forEach (function (mod) {
        console.log ('[' + moduleName + '] Info: - %s %s%s -> %s',
                     mod,
