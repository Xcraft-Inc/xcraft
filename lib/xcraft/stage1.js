...
installStrongDeps (function () {
  ...
  async.series ([
    function (callback) {
      ...
      console.log ('[stage1] Info: core packages publication');
      execXcraft ('publish', [], callback);
    }, function (callback) {
      console.log ('[stage1] Info: core packages installation');
      ...
      const list = Object.keys (binList).filter ((bin) => {
        return new RegExp (binList[bin]).test (process.platform);
      });
      execXcraft ('bin', list, callback);
    }, function (callback) {
